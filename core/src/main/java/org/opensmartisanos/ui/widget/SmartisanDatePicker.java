...
public class SmartisanDatePicker extends FrameLayout {
    ...
    private static final String LOG_TAG = "SmartisanDatePicker";
    private Calendar mCurrentDate;
    private Locale mCurrentLocale;
    private final DateFormat mDateFormat;
    private final SmartisanNumberPicker mDaySpinner;
    private boolean mIsEnabled;
    private Calendar mMaxDate;
    private Calendar mMinDate;
    private final SmartisanNumberPicker mMonthSpinner;
    ...
    private final SmartisanNumberPicker mYearSpinner;
    ...
    public interface OnDateChangedListener {
        void onDateChanged(SmartisanDatePicker smartisanDatePicker, int i, int i2, int i3, DatePickerType datePickerType);
    }
    ...
    public SmartisanDatePicker(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SmartisanDatePicker(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        this.mDateFormat = new SimpleDateFormat(DATE_FORMAT);
        this.mIsEnabled = true;
        setCurrentLocale(Locale.getDefault());
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        inflater.inflate(R.layout.smartisan_rom_date_picker, (ViewGroup) this, true);
        ...
        SmartisanNumberPicker.OnValueChangeListener onChangeListener = new SmartisanNumberPicker.OnValueChangeListener() { // from class: smartisanos.widget.SmartisanDatePicker.1
            ...
            public void onValueChange(SmartisanNumberPicker picker, int oldVal, int newVal) {
                SmartisanDatePicker.this.mTempDate.setTimeInMillis(SmartisanDatePicker.this.mCurrentDate.getTimeInMillis());
                if (picker == SmartisanDatePicker.this.mDaySpinner) {
                    SmartisanDatePicker.this.mTempDate.set(5, newVal);
                } else if (picker == SmartisanDatePicker.this.mMonthSpinner) {
                    SmartisanDatePicker.this.mTempDate.set(2, newVal);
                    SmartisanDatePicker.this.mTempDate.set(5, 1);
                } else if (picker == SmartisanDatePicker.this.mYearSpinner) {
                    SmartisanDatePicker.this.mTempDate.set(1, newVal);
                    SmartisanDatePicker.this.mTempDate.set(5, 1);
                } else {
                    throw new IllegalArgumentException();
                }
                int daySpinnerValue = SmartisanDatePicker.this.mDaySpinner.getValue();
                SmartisanDatePicker smartisanDatePicker = SmartisanDatePicker.this;
                smartisanDatePicker.setDate(smartisanDatePicker.mTempDate.get(1),
                        SmartisanDatePicker.this.mTempDate.get(2),
                        SmartisanPickerMath.normalizeDayOrFirst(daySpinnerValue,
                                SmartisanDatePicker.this.mTempDate.getActualMaximum(5)));
                SmartisanDatePicker.this.updateSpinners();
                SmartisanDatePicker.this.notifyDateChanged();
            }
        };
        this.mSpinners = (LinearLayout) findViewById(R.id.smartisan_rom_pickers);
        SmartisanNumberPicker smartisanNumberPicker = (SmartisanNumberPicker) findViewById(R.id.smartisan_rom_day);
        this.mDaySpinner = smartisanNumberPicker;
        smartisanNumberPicker.setFormatter(SmartisanNumberPicker.getTwoDigitFormatter());
        this.mDaySpinner.setOnValueChangedListener(onChangeListener);
        this.mDaySpinner.setTextSize(SmartisanLocaleUtils.dp(getContext(), 16.0f), SmartisanLocaleUtils.dp(getContext(), 18.0f));
        this.mDaySpinner.setTextColor(normalColor, selectColor);
        SmartisanNumberPicker smartisanNumberPicker2 = (SmartisanNumberPicker) findViewById(R.id.smartisan_rom_month);
        this.mMonthSpinner = smartisanNumberPicker2;
        ...
        this.mMonthSpinner.setTextSize(SmartisanLocaleUtils.dp(getContext(), 16.0f), SmartisanLocaleUtils.dp(getContext(), 18.0f));
        this.mMonthSpinner.setTextColor(normalColor, selectColor);
        SmartisanNumberPicker smartisanNumberPicker3 = (SmartisanNumberPicker) findViewById(R.id.smartisan_rom_year);
        this.mYearSpinner = smartisanNumberPicker3;
        smartisanNumberPicker3.setOnValueChangedListener(onChangeListener);
        this.mYearSpinner.setTextSize(SmartisanLocaleUtils.dp(getContext(), 16.0f), SmartisanLocaleUtils.dp(getContext(), 18.0f));
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(SmartisanDatePicker.class.getName());
    }

    @Override // android.view.View
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(SmartisanDatePicker.class.getName());
    }
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        // Match the R2 restore path: the instance has already been initialized, so restoring the
        // selected date must preserve any min/max range configured by the caller.
    ...
    private void updateSpinners() {
        ...
        if (AnonymousClass2.$SwitchMap$smartisanos$widget$SmartisanDatePicker$DatePickerType[this.mPickerType.ordinal()] == 1) {
    ...
    /* JADX INFO: renamed from: smartisanos.widget.SmartisanDatePicker$2, reason: invalid class name */
    static /* synthetic */ class AnonymousClass2 {
        static final /* synthetic */ int[] $SwitchMap$smartisanos$widget$SmartisanDatePicker$DatePickerType;

        static {
            int[] iArr = new int[DatePickerType.values().length];
            $SwitchMap$smartisanos$widget$SmartisanDatePicker$DatePickerType = iArr;
            try {
                iArr[DatePickerType.BIRTHDAY.ordinal()] = 1;
            } catch (NoSuchFieldError e) {
            }
            try {
                $SwitchMap$smartisanos$widget$SmartisanDatePicker$DatePickerType[DatePickerType.EVENT.ordinal()] = 2;
    ...
    private static class SavedState extends View.BaseSavedState {
        public static final Parcelable.Creator<SavedState> CREATOR = new Parcelable.Creator<SavedState>() { // from class: smartisanos.widget.SmartisanDatePicker.SavedState.1
